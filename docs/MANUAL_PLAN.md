...
> validation (2026-07-20, spoiler policy + first slice confirmed).
...
- **Text is i18n-keyed** under `t.codex.*` (CLAUDE.md §23) — English first,
  structured for translation. Prose never embeds a value; it interpolates the
  imported constant at render time.
...
- **Entry point:** a "Codex" button on the left rail of `GameShell`
  (`ls-rail-button` pattern), reachable from **every** screen. Also openable
  via a `?` affordance and a keyboard shortcut.
...
- **Contextual deep-link:** opening from a screen jumps to the relevant
  section — PlanetView → deposits / buildings / population; GalaxyMap → travel /
  telescopes; Market → census / trading. The current `view.kind` selects the
  default section.
...
- **UI captures:** generated from the real client via the existing Playwright
  ...
  (§7, §16). Deferred past the first slice; first slice ships with SVG diagrams.
...
2. ★ **Population** — three ages (children/actives/seniors), aging on fixed
   ...
   rations, oxygen on hostile climates; death clocks; over-capacity risk.
   Sources: `popv2.ts` (`*_DAYS`, `NATALITY_BY_RESIDENTIAL`, `RATION_CS`,
   `OXYGEN_PER_1000_PER_DAY`, `CLOCK_DAYS`, over-cap coefs), `popCap`.
...
- **Unit:** section registry integrity (every section id resolves an i18n key +
  renderer); the number-binding test (each documented constant renders its live
  `@atg/shared` value).
- **E2E (Playwright):** Codex opens from GalaxyMap, PlanetView and Market;
  ...
  render their headings and an interactive "Exact rule" block toggles.
...
- No hand screenshots (SVG diagrams first; captured UI later).
